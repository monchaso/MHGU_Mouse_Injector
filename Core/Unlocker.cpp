...
bool Unlocker::Hook(uintptr_t address) {
  ...
  targetAddress = address; // Update target in case it changed (unlikely for
                           // reuse, but safe)

  // Only allocate trampoline ONCE.
  if (trampolineAddress == 0) {
    ...
    // We read them from the process to be 100% sure we match exact verified
    // bytes
    std::vector<uint8_t> originalShellcodeBytes(STOLEN_SIZE);
    mem->ReadBuffer(targetAddress, originalShellcodeBytes.data(), STOLEN_SIZE);

    // Append original message bytes to shellcode
    // IMPORTANT: We want to DISABLE the original camera write instruction
    // (first 3 bytes: 89 04 2E) so the game doesn't fight our injection. Bytes
    // 0-2: mov [rsi+rbp],eax  -> NOP these! Bytes 3-15: Other instructions ->
    // Keep these!

    for (size_t i = 0; i < originalShellcodeBytes.size(); ++i) {
      if (i < 3)
        shellcode.push_back(0x90); // NOP the write
      else
        shellcode.push_back(originalShellcodeBytes[i]);
    }
  ...
  // ALWAYS: Install the JMP Hook
  // We need to support 'Re-Enabling', so we must write the JMP to Target even
  // if trampoline exists. Also need to save original bytes for 'Restore'.

  // Read current bytes at target (could be Original or already Hooked? Check
  // duplicates handles it) Since we checked duplicates at start, we assume it's
  // NOT currently hooked. So 'targetAddress' should contain original bytes
  // (because we Restored). Read them to save for Patch.
  const size_t STOLEN_SIZE = 16;
  std::vector<uint8_t> originalBytes(STOLEN_SIZE);
  if (!mem->ReadBuffer(targetAddress, originalBytes.data(), STOLEN_SIZE)) {
    std::cerr << "[Unlocker] Failed to read original bytes for hook!"
              << std::endl;
    return false;
  }
